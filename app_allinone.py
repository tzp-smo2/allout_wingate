...
st.title("Analyse complète test all-out 30s (SmO2 + puissance)")

st.sidebar.header("1. Fichiers à charger")
...
logo = "logo_TZP.jpg"  # doit être dans ton dépôt GitHub
...
if data_file:
    ...
    if df.empty:
        st.error("Les colonnes sélectionnées ne contiennent pas de données exploitables.")
    else:
        ...
        st.write(df[["SmO2", "Puissance"]].describe())

        st.subheader("Graphique SmO2 + Puissance")
        fig, ax1 = plt.subplots()
        ax1.set_title("SmO2 & Puissance pendant le test")
