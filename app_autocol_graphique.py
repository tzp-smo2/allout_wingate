...
st.title("Analyse all-out 30s avec SmO2 + Puissance (détection automatique des colonnes)")
...
if data_file:
    ...
    if not all([time_col, smo2_col, power_col]):
        st.error("Impossible de détecter automatiquement toutes les colonnes nécessaires.")
