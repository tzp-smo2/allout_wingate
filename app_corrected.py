...
st.title("Analyse test all-out 30" (SmO₂ + puissance)")

# --- Chargement des fichiers ---
st.sidebar.header("Fichiers")
data_file = st.sidebar.file_uploader("Données SmO₂ / Puissance", type=["csv", "xlsx"])
...
if data_file:
    ...
    smo2_col_1 = st.selectbox("SmO₂ Capteur 1", df.columns)
    smo2_col_2 = st.selectbox("SmO₂ Capteur 2 (optionnel)", ["Aucun"] + list(df.columns))
    ...
    ax1.plot(df["Temps"], df["SmO2_1"], label="SmO₂ capteur 1", color='blue')
    if "SmO2_2" in df:
        ax1.plot(df["Temps"], df["SmO2_2"], label="SmO₂ capteur 2", color='cyan')
    ax1.set_ylabel("SmO₂ (%)")
    ax2 = ax1.twinx()
    ...
    st.write({
        ...
        "SmO₂ min": round(sm_min,1), "Pente T2 SmO₂ (%/s)": round(slope_T2,3),
        "Pente T4 SmO₂ (%/s)": round(slope_T4,3),
    ...
    if st.button("Générer rapport PDF") and info_file:
        ...
        metrics = [
            ...
            ("SmO₂ min", sm_min),
            ("Pente T2 SmO₂", slope_T2),
            ("Pente T4 SmO₂", slope_T4)
        ]
