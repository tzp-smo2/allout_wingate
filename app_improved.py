...
if data_file:
    ...
    result_table = pd.DataFrame({
        "Mesure": [
            "Puissance moyenne 30s", "Moyenne 0-10s", "Moyenne 10-20s", "Moyenne 20-30s",
            "Pmax", "Pmin", "ΔP", "Fatigue Index (%)", "Temps perte puissance",
            "SmO2 min", "Pente T2 SmO2 (%/s)", "Pente T4 SmO2 (%/s)", "T½ Réoxygénation"
        ],
        "Valeur": [
            round(mean_p,1), round(p10[0],1), round(p10[1],1), round(p10[2],1),
            int(p_max), int(p_min), int(delta_p), round(fatigue_index,1),
